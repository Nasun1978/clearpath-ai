...
export async function POST(request: NextRequest) {
  ...
  if (DEMO_MODE) {
    ...
    return NextResponse.json({
      ...
      summary: `ClearPath AI analyzed ${proposal.project_name} against ${checks.length} compliance requirements. ${checks.filter((c: Record<string, string>) => c.result === "pass").length} checks passed, ${checks.filter((c: Record<string, string>) => c.result === "fail").length} failed, ${checks.filter((c: Record<string, string>) => c.result === "needs_review").length} require human review.`,
    });
  }
