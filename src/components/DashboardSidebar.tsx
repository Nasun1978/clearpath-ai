...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/dashboard/lihtc-closing", label: "LIHTC Closing", icon: <ClosingIcon />, accent: "teal" },
    ],
  },
  {
    label: "Analysis",
    items: [
      ...
      { href: "/dashboard/rent-roll", label: "Rent Roll Analyzer", icon: <RentRollIcon /> },
      ...
      { href: "/dashboard/voucher-map", label: "Voucher & LIHTC Map", icon: <VoucherMapIcon />, accent: "teal" },
    ],
  },
  {
    label: "Marketplace",
...
export default function DashboardSidebar({ deadlineCount = 0, onBellClick, onSearchClick }: DashboardSidebarProps) {
    ...
    >
      ...
      <div className="px-2 py-2 border-b border-slate-800/60">
        <button
          ...
          className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg bg-slate-800/60 border border-slate-700/60 text-slate-500 hover:text-slate-300 hover:border-slate-600 transition-colors ${collapsed ? "justify-center" : ""}`}
          title="Search (⌘K)"
        >
          ...
          {!collapsed && (
            <>
              ...
              <kbd className="text-[10px] font-mono bg-slate-900 border border-slate-700 px-1 py-0.5 rounded">⌘K</kbd>
            </>
          )}
        </button>
      </div>
      ...
      {/* Admin — pinned above bottom bar, always visible */}
      <div className="shrink-0 border-t border-amber-900/30 bg-amber-950/20 px-2 py-2 space-y-0.5">
        {!collapsed && (
          <p className="px-2 mb-1 text-[10px] font-semibold text-amber-700 uppercase tracking-wider">Admin</p>
        )}
        {[
          ...
          <Link
            ...
            className={`flex items-center gap-2.5 px-2 py-2 rounded-lg text-sm font-medium transition-colors ${
              isActive(item.href)
                ? "bg-amber-900/50 text-amber-300 border-l-2 border-amber-500"
                : "text-amber-600 hover:bg-amber-900/30 hover:text-amber-300"
            } ${collapsed ? "justify-center" : ""}`}
          >
            <span className="shrink-0">{item.icon}</span>
            {!collapsed && <span className="truncate">{item.label}</span>}
